...
########## INPUT DYNAMIC DATA ##########
dynamic = read.csv("/Users/u1058369/Library/CloudStorage/Dropbox/Pieter/Collaborations/Elliott Schmidt/dynamic_input.csv")
...
t_coma = c(1, 10, 60, 240, 1440, 4320, 10080, 20160, 30240, 40320) 
# 1 min, 10 min, 1 hour, 1 day, 3 days, 1 week, 2 weeks, 3 weeks, 4 weeks
...
#extra_ramprates = c(0.5, 1, 5)/60 #example
...
for(i in 1:length(groups))
  ...
  Tc = 34 #example. This can be group specific
  ...
  for(j in 1:length(t_coma))
    ...
    if(length(ramprates)==1)
      ...
      # z = 2.347 # Orsted method estimate
      z = 2.061 # Rezende method estimate
...
write.csv(sCTmax.predictions,file="/Users/u1058369/Library/CloudStorage/Dropbox/Pieter/Collaborations/Elliott Schmidt/sCTmax_t_coma_Rezende.csv")
...
# Comparing predictions with different estimates of z
# Rezende: z = 2.06
rezende_pred <- read.csv("/Users/u1058369/Library/CloudStorage/Dropbox/Pieter/Collaborations/Elliott Schmidt/sCTmax_t_coma_Rezende.csv")
head(rezende_pred)
orsted_pred <- read.csv("/Users/u1058369/Library/CloudStorage/Dropbox/Pieter/Collaborations/Elliott Schmidt/sCTmax_t_coma_Orsted.csv")
head(orsted_pred)

comb_dat <- data.frame(cbind(rezende_pred,orsted_pred[,4:5]))
comb_dat$delta_pred <- comb_dat$sCTmax_pred - comb_dat$sCTmax_pred.1

library(tidyr)
comb_dat_long <- comb_dat %>% pivot_longer(cols = c(4,6), names_to = "Method", values_to = "sCTmax")
comb_dat_long$Method <- dplyr::recode(comb_dat_long$Method, 
                                       "sCTmax_pred" = "Rezende", 
                                       "sCTmax_pred.1" = "Orsted")

ggplot(comb_dat_long, aes(x = t_coma, y = sCTmax, colour = group, linetype = Method)) +
  geom_point() +
...
ggplot(sCTmax.predictions_extra, aes(x = log10(extra_t_coma_pred), y = extra_sCTmax, colour = group)) +
  #facet_wrap(~group) +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_colour_viridis_d() +
  labs(x = bquote(italic(t)[coma]~(log[10]~minutes)), y = bquote(Predicted~sCTmax~(degree*C))) +
  theme_bw()



ggplot(sCTmax.predictions, aes(x = t_coma, y = sCTmax_pred, colour = group)) +
 # facet_wrap(~group) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ log10(x), se = F) +
  scale_colour_viridis_d() +
