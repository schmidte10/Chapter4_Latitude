...
```{r eda}
ggplot(growth3, aes(x=DEV_TEMP, y=LENGTH, color=FEMALE, fill=FEMALE)) + 
...
# Model1.1
```{r model1.1}
model1.1 <- lmer(LENGTH ~ as.numeric(DEV_TEMP) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION), 
                 REML=FALSE,
                 data=growth3)   
...
ggplot(dev_temp_pred, aes(x = DEV_TEMP, y = fit1.1)) +
  geom_jitter(data = growth3, aes(y = LENGTH, colour = REGION), width=0.2) +
  geom_line(size = 2) +
  theme_classic() + 
  facet_wrap(~REGION)
``` 
...
#Model1.2
```{r model1.2}
model1.2 <- lmer(LENGTH ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE)+ scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION), 
                 REML=FALSE,
                 data=growth3)  
...
# Model1.3
```{r model1.3}
model1.3 <- lmer(LENGTH ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1 |FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
# Model1.4 
```{r}
model1.4 <- lmer(LENGTH ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE)+ scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1+DEV_TEMP|FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
# Model1.5 
```{r warning=FALSE}
model1.5 <- lmer(LENGTH ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1+DEV_TEMP + I(DEV_TEMP^2)|FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
```
```{r eval =FALSE}
pdf("./figures/S_BLUPSoutput_standardlength.pdf", width = 8, height =5)
ggplot(genotype_data, aes(genotype_ordered, genotype_slopes)) +
  geom_bar(stat = "identity", aes(group = genotype, fill = pop)) +
  xlab("Genotype (in ranked order of plasticity)") +
  ylab("Plasticity (population-average + BLUP slope estimate)") +
