...
datasummary(REGION *(LOE_MEAN_EAS) ~  N + mean + MinMax + Range + Histogram, 
            data=ctmax2)
datasummary(REGION * DEV_TEMP * (LOE_MEAN_EAS) + (LOE_MEAN_EAS) ~  N + median + MinMax + Range + Histogram, 
            data=ctmax2)
...
```{r echo=FALSE}
model1.1 <- readRDS("04.CTmax_data_analysis_files/model1.1.RDS")
```
...
#### MCMC diagnositics 
```{r mcmc-diag-model1.1a, fig.height=9}
...
```{r echo=FALSE}
model1.2 <- readRDS("04.CTmax_data_analysis_files/model1.2.RDS")
```
...
```{r echo=FALSE}
model1.3 <- readRDS("04.CTmax_data_analysis_files/model1.3.RDS")
```
...
### Model1.4 {.tabset}
```{r model1.4, cache=TRUE, eval=FALSE}
f.model1.4 <- bf(LOE_MEAN_EAS ~ 1 + DEV_TEMP*REGION + 
                       scale(MASS, center=TRUE, scale=TRUE) + 
                   scale(AGE_DAYS, center=TRUE, scale=TRUE) +
                       (1| FEMALE) + (1| TANK), 
                     family=gaussian())

model1.4 <- brm(f.model1.4,
              data = ctmax2,
              prior =ctmax.priors,
              warmup = 500, 
              iter = 5000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95))
saveRDS(model1.4, file="./04.CTmax_data_analysis_files/model1.4.RDS")
```

```{r echo=FALSE}
model1.4 <- readRDS("04.CTmax_data_analysis_files/model1.4.RDS")
```

#### DHARMa 
```{r model1.1-dharma}
#--- distribution check ---#
pp_check(model1.4, type = 'dens_overlay_grouped', ndraws=150, group="REGION")

#--- DHARMa checks ---#
preds <- posterior_predict(model1.4, ndraws=250, summary=FALSE)
model1.4_resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(ctmax2$LOE_MEAN_EAS), 
                              fittedPredictedResponse = apply(preds, 2, mean), 
                              integerResponse = 'student')
plot(model1.4_resids) ; testDispersion(model1.4_resids)
```

#### MCMC diagnositics 
```{r mcmc-diag-model1.1, fig.height=9}
stan_trace(model1.4$fit, allpars=TRUE)
stan_ac(model1.4$fit) 
stan_rhat(model1.4$fit) 
stan_ess(model1.4$fit)
stan_dens(model1.4$fit, separate_chains = TRUE)
```
#### {-}

### LOO
```{r model-comparisons-3, cache=TRUE}
LOO(model1.1,model1.2, model1.3, model1.4) 
```
...
```{r emmeans-pairwise}
model1.1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="DEV_TEMP") |> summary()
model1.1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="REGION") |> summary()
model1.1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="REGION") |> confint()
model1.1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response")
out <- model1.1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") 
out$emmeans |>
  as.data.frame()
```
...
ctmax.plot <- int_draws_plotting |> 
  ...
  geom_hline(yintercept = median(int_draws_plotting$CTMAX), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) +
  ...
  annotate("text", x=0.6,y=37.06, label = paste0(round(median(int_draws_plotting$CTMAX), 2)," \u00B0","C"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
...
ggsave(file="./figures/figure3.svg",  plot=ctmax.plot, width=8, height=5, units = "in", dpi=300) 
ggsave(file="./figures/figure3.pdf",  plot=ctmax.plot, width=8, height=5, units = "in", dpi=1200)
```