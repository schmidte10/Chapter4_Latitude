...
```{r setwd-1, echo=FALSE, eval=FALSE}
...
save(growth2, file ="./import_files/growth2.RData")
```
...
datasummary(REGION *(LENGTH + MASS) + (LENGTH + MASS) ~  N + mean + MinMax + Range + Histogram, data=growth3)

datasummary(REGION * DEV_TEMP * (LENGTH + MASS) + (LENGTH + MASS) ~  N + median + MinMax + Range + Histogram, 
            data=growth3)

growth3 |> 
  group_by(REGION, DEV_TEMP) |> 
  summarise(n_tanks =n_distinct(FEMALE))

growth3 |> 
  group_by(REGION, DEV_TEMP) |> 
  summarise(n_tanks =n_distinct(TANK))
```
...
### Priors 
```{r priors-re0.2}
model.re0.2.formula <- bf(LENGTH ~ 1 + (1|TANK) + (1|FEMALE))
get_prior(model.re.formula, data=growth3)
growth.priors <- prior(normal(31.4, 1.5), class="Intercept") +  
  prior(student_t(3, 0, 4.4), class = "sigma")
```
...
### Model-2
```{r fit-model-re0.2, cache=TRUE} 
model.re0.2 <- brm(model.re0.2.formula,
              data = growth3, 
              prior = growth.priors,
              warmup = 500, 
              iter = 5000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95)) 
prior_summary(model.re0.2) 
saveRDS(model.re, file="./01.growth_data_analysis_files/model.re0.2.length.RDS")
```

### model validation 
```{r model-diag-re0.2}
stan_trace(model.re0.2 $fit)
stan_ac(model.re0.2 $fit) 
stan_rhat(model.re0.2 $fit) 
stan_ess(model.re0.2 $fit)
stan_dens(model.re0.2 $fit, separate_chains = TRUE)
```
### {-} 
...
### Priors 
```{r priors-re0}
model.re0.formula <- bf(LENGTH ~ 1 + (1|TANK) + (1|FEMALE) + (1|POPULATION))
get_prior(model.re.formula, data=growth3)
growth.priors <- prior(normal(31.4, 1.5), class="Intercept") +  
  prior(student_t(3, 0, 4.4), class = "sigma")
```
...
### Model 3
```{r fit-model-re0, cache=TRUE} 
model.re0 <- brm(model.re0.formula,
              data = growth3, 
              prior = growth.priors,
              warmup = 500, 
              iter = 5000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95)) 
prior_summary(model.re0) 
saveRDS(model.re, file="./01.growth_data_analysis_files/model.re0.length.RDS")
```

### model validation 
```{r model-diag-re0}
stan_trace(model.re0$fit)
stan_ac(model.re0$fit) 
stan_rhat(model.re0$fit) 
stan_ess(model.re0$fit)
stan_dens(model.re0$fit, separate_chains = TRUE)
```
...
### Priors 
```{r priors-re2}
model.re2.formula <- bf(LENGTH ~ 1 + (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE) + (1|CLUTCH_ORDER))
get_prior(model.re.formula, data=growth3)
growth.priors <- prior(normal(31.4, 1.5), class="Intercept") +  
  prior(student_t(3, 0, 4.4), class = "sigma")
```
...
saveRDS(model.re, file="./01.growth_data_analysis_files/model.re2.length.RDS")
```
...
## LOO
```{r model-comparisons-loo}
LOO(model.re0,model.re, model.re2,model.re0.2) 
```
## {-}
...
Also because we are doing Bayesian modelling we will be listing priors. Our length data is normally distributed as shown in some of the exploratory data analysis plots. We can confirm this assumption during the model validation stage as well. 
...
```{r echo =FALSE}
model1 <- readRDS("01.growth_data_analysis_files/model1.length.RDS")
```
...
saveRDS(model.mat, file="./01.growth_data_analysis_files/model.maternal.length.RDS")
```

```{r echo=FALSE}
model.mat <- readRDS("01.growth_data_analysis_files/model.maternal.length.RDS")
```

### stan plots
```{r mcmc-diag-model.mat-2, fig.height=9}
...
saveRDS(model.geo, file="./01.growth_data_analysis_files/model.geographic.length.RDS")
```


```{r echo=FALSE}
model.geo <- readRDS("01.growth_data_analysis_files/model.geographic.length.RDS")
```


### stan plots
```{r mcmc-diag-modelgeo-2, fig.height=9}
...
## LOO
```{r model-comparisons-loo2}
LOO(model1, model.geo) 
```


Models show very little difference between each other, therefore we will continue with the most parismonous model - **model1**.
...
model1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="DEV_TEMP") |> summary()

model1 |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() |> summary()

model1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="REGION") |> summary()

model1 |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() |> pairs() |> summary()
```
...
length.plot <- int_draws_plotting |> 
  ...
  geom_hline(yintercept = median(int_draws_plotting$LENGTH), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) + 
  ggdist::stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), 
  ...
  annotate("text", x=6.8,y=32.4, label = paste0(round(median(int_draws_plotting$LENGTH), 2)," (mm)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
...
```{r}
growth_age_plot <- ggplot(growth3, aes(x=REGION, y=AGE_DAYS, fill=REGION, color=REGION)) + 
  ...
  scale_x_discrete(labels =c("leading" = "High", 
                             "core" = "Low")) + 
  theme_classic() +
  labs(
    x = "LATITUDE",
...
#ggsave(file="./figures/length.age.plot.svg",  plot=growth_age_plot, width=8, height=5, units = "in", dpi=300) 
#ggsave(file="./figures/length.age.plot.pdf",  plot=growth_age_plot, width=8, height=5, units = "in", dpi=300)
```
