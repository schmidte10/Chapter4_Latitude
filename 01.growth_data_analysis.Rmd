...
model1.re <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY + cAGE_DAYS  + (1|LEVEL/TANK) + (1|REGION/POPULATION) + (1|FEMALE),
              family=gaussian(),
              data = growth2, 
              warmup = 1500, 
              iter = 5000,
...
#--- run model ---#
model1.re.wo <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY + cAGE_DAYS + (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE),
              family=gaussian(),
              data = growth3, 
              warmup = 1500, 
              iter = 5000,
...
## probabilities 
```{r probabilities}
model1 |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 31.5, length.out = 10)) 
model1 |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 28.5, length.out = 10)) 
model1 |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 30, length.out = 10)) 
```
# {-}
...
```{r summary-figure-1}
var <- get_variables(model1); var
var1 <- get_variables(model1)[c(1:4,8:9)] ; var1 
