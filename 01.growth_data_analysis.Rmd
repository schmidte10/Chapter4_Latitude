...
model1 <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY,
              family=gaussian(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
model.mat <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY + DAYS_IN_TREATMENT + EGG_COUNT + HATCHING_SUCCESS + CLUTCH_ORDER,
              family=gaussian(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
model.geo <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY + FEMALE_LAT,
              family=gaussian(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
model1.rf <- brm(cLENGTH ~ DEV_TEMP*REGION + cFEMALE_STANDARD_LENGTH + cDENSITY + (1|TANK) + (1|LEVEL) + (1|FEMALE),
              family=gaussian(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
pp_check(model1.rf, type = 'dens_overlay')

preds <- posterior_predict(model1.rf, ndraws=250, summary=FALSE)
model1.rf.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(growth2$cLENGTH), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.rf.resids) 

testOutliers(model1.rf)
```
