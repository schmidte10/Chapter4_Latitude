...
```{r fit-model-1-3}
standist::visualize("student_t(3,0.135,3.8325)", xlim=c(-10,10))
standist::visualize("student_t(3,0,3.8325)", xlim=c(-10,10))
```
...
* for the intercept, it is using a student t(flatter normal) prior with a mean of 0.2 and a standard deviation of 3.9. These values are derived from the median and median absolute deviation of the response variable 
...
```{r fit-model-2-3}
standist::visualize("student_t(3,0.2,3.9)", xlim=c(-10,100))
standist::visualize("student_t(3,0,3.9)", xlim=c(-10,100))
```
...
mcmc_plot(model.mat, type='neff_hist') # one ratio is <0.5 may have to tigthen up the prior for this 
```
...
```{r fit-model3-3}
standist::visualize("student_t(3,0.1,3.8)", xlim=c(-10,100))
standist::visualize("student_t(3,0,3.8)", xlim=c(-10,100))
```
...
mcmc_plot(model.geo, type='neff_hist') # one ratio is <0.5 may have to tigthen up the prior for this 
```
...
Before we run the model through the model validation steps we will add in our random factors. Our random factors will include **LEVEL**, and **CLUTCH_NUMBER**. 
...
#--- distribution check ---#
pp_check(model1.re.wo, type = 'dens_overlay')
...
testDispersion(model1.re.wo.resids)
``` 
...
```{r summary-100}
summary(model1.re.wo)
```

## R2 

```{r r-squared}
model1.re.wo |> bayes_R2(summary = FALSE) |> median_hdci()
```

## tidyMCMC 

```{r tidy}
tidyMCMC(model1.re.wo, estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
```{r bayesplot-1}
model1.re.wo |> mcmc_plot(type='intervals')
```

## emmeans - pariwise 

```{r emmeans-pairwise}
model1.re.wo |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="DEV_TEMP") |> summary()
```

## probabilities 
```{r probabilities}
model1.re.wo |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 31.5, length.out = 10)) |> pairs(type = 'response')
...
```{r summary-figure-1}
var <- get_variables(model1.re.wo); var
var1 <- get_variables(model1.re.wo)[c(1:4,8:9)] ; var1 
