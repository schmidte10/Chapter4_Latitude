...
```{r outlier-removal-1, warning=FALSE, message=FALSE, echo=FALSE, fig.height=9, fig.width=9}
eda1
```
...
### summary figure {.tabset}

#### Manual

```{r summary-figure-1}
var <- get_variables(model1.re.wo); var
var1 <- get_variables(model1.re.wo)[c(1:3,8:9)] ; var1 
...
int_draws_plotting |> 
  ...
  stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), point_interval = mode_hdci, .width = c(.66, .90, .95)) + 
  scale_fill_brewer(na.translate = FALSE) +
  scale_fill_manual(values = c("#D1E5F0" ,"#67A9CF", "#2166AC","#FDDBC7","#EF8A62", "#B2182B"))+
