...
## Priors 
```{r priors-2}
model.re.formula <- bf(MASS ~ 1 + (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE))
...
saveRDS(model.re, file="./02.mass_data_analysis_files/model.randomfactors.RDS")
```
...
## Priors 
```{r priors-2}
model.re2.formula <- bf(MASS ~ 1 + (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE) + (1|CLUTCH_ORDER))
get_prior(model.re.formula, data=growth3)
mass.priors.re <- prior(normal(1.13, 0.20), class="Intercept") +  
  prior(student_t(3, 0, 0.53), class = "sigma")
```
...
## Model 2 - random factors
```{r fit-model-1, cache=TRUE} 
model.re2 <- brm(model.re2.formula,
              data = growth3, 
              prior = mass.priors.re,
              warmup = 500, 
              iter = 5000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95)) 
prior_summary(model.re2) 
saveRDS(model.re2, file="./02.mass_data_analysis_files/model.randomfactors2.RDS")
```

## model validation [random factors]
```{r model-diag-1}
stan_trace(model.re2$fit)
stan_ac(model.re2$fit) 
stan_rhat(model.re2$fit) 
stan_ess(model.re2$fit)
stan_dens(model.re2$fit, separate_chains = TRUE)
```

### LOO
```{r model-comparisons-2}
LOO(model.re, model.re2) 
```
...
saveRDS(model1, file="./02.mass_data_analysis_files/model1.mass.RDS")
```
...
model1.formula <- bf(MASS ~ 1 + 
                         ...
                         scale(AGE_DAYS, center=TRUE, scale=TRUE) +
                         scale(LENGTH, center=TRUE, scale=TRUE) + 
                         scale(DAYS_INT_TREATMENT, center=TRUE, scale=TRUE) + 
                         scale(EGG_COUNT, center=TRUE, scale=TRUE) + 
                         scale(HATCHING_SUCCESS, center=TRUE, scale=TRUE) + 
                         (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE), 
                         family=gaussian())
get_prior(model1.formula, data=growth3)
mass.priors <- prior(normal(1.13, 0.20), class="Intercept") +  
...
```{r random-factors-2, cache=TRUE, eval=FALSE, echo=FALSE}
...
```{r model1.mass.log, eval=FALSE, echo=FALSE}
#--- distribution check ---#
...
Not really, in fact the log link perhaps makes it worse! Maybe we can try a student's t distribution which has fatter tails and may deal with residuals a bit better. 

# Student Model
```{r random-factors-3, cache=TRUE, eval=FALSE, echo=FALSE}
model1.mass.st <- brm(cMASS ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=student,
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.9))  
summary(model1.mass.st)
```

Lets see if the model has improved 

```{r model1.mass.student, eval=FALSE, echo=FALSE}
#--- distribution check ---#
pp_check(model1.mass.st, type = 'dens_overlay')

#--- DHARMa checks ---#
growth3 <- growth2 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.mass.st, ndraws=250, summary=FALSE)
model1.mass.st.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(growth3$cMASS), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.mass.st.resids) ; testDispersion(model1.mass.st.resids)
```

The student t distribution doesn't change the results. In all the models the observed values seem to lower than expected, and the values at the lower end seem to have issues with their residuals. There may be an issue with how the data is being modelled. So here is what I will try moving forward: 

1. Center MASS on median value instead of Mean 
2. scale numeric co-variates including cLength, hatching success, and density. 

# Median-centered model
Lets start with the first option 

```{r re-fit-model-3, eval=FALSE, echo=FALSE}
growth4 <- growth2 |> 
  mutate(mdMASS = scale(MASS, center = median(MASS), scale=FALSE)[,1])

``` 

Let's try out gaussian model to see if this improves the outcome 

```{r re-fit-model-5, eval=FALSE, echo=FALSE}
model1.mdMASS <- brm(mdMASS ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=gaussian,
              data = growth4, 
              warmup = 4000, 
              iter = 10000,
...
```{r model.mdMASS, eval=FALSE, echo=FALSE}
#--- distribution check ---#
pp_check(model1.mdMASS, type = 'dens_overlay')

#--- DHARMa checks ---#
growth5 <- growth4 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.mdMASS, ndraws=250, summary=FALSE)
model1.mdMASS.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(growth5$mdMASS), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.mdMASS.resids) ; testDispersion(model1.mdMASS.resids)
```

Doesn't seem to help 

# scaled model
```{r model1.scaled, eval=FALSE, echo=FALSE}

growth4 <- growth4 |> 
  mutate(HATCHING_SUCCESS = scale(HATCHING_SUCCESS, center=TRUE, scale = TRUE), 
         cDENSITY = scale(cDENSITY, center=FALSE, scale=TRUE), 
         cLength = scale(cLENGTH, center=FALSE, scale=TRUE))
model1.scaled <- brm(cMASS ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=gaussian,
              data = growth4, 
              warmup = 4000, 
              iter = 10000,
...
pp_check(model1.scaled, type = 'dens_overlay')

#--- DHARMa checks ---#
growth5 <- growth4 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.scaled, ndraws=250, summary=FALSE)
model1.scaled.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(growth5$mdMASS), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.scaled.resids) ; testDispersion(model1.scaled.resids)
```

This makes the model worse. Out of ideas for now. 

# log-transformed model
```{r model1.tlog, eval=FALSE, echo=FALSE}
model1.tlog <- brm(log(cMASS) ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=gaussian,
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
pp_check(model1.tlog, type = 'dens_overlay')

#--- DHARMa checks ---#
growth3 <- growth2 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.tlog, ndraws=250, summary=FALSE)
model1.tlog.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(log(growth3$cMASS)), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.tlog.resids) ; testDispersion(model1.tlog.resids)
``` 

# Square-root transformed model 
```{r model1.tsqrt, eval=FALSE, echo=FALSE}
model1.tsqrt <- brm(sqrt(cMASS) ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=gaussian(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
summary(model1.tsqrt)
pp_check(model1.tsqrt, type = 'dens_overlay')

#--- DHARMa checks ---#
growth3 <- growth2 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.tsqrt, ndraws=250, summary=FALSE)
model1.tsqrt.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(sqrt(growth3$cMASS)), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.tsqrt.resids) ; testDispersion(model1.tsqrt.resids)
``` 

The square root transformation seems to help with the lower end residuals. There are still some issues with the data but it is looking much better. We may try some slight adjustments but over all it looks pretty good. 

# Square-root transformed model with student distribution
```{r model1.tsqrt.student, cache=TRUE}
model1.tsqrt.student <- brm(sqrt(cMASS) ~ DEV_TEMP*REGION + HATCHING_SUCCESS + cDENSITY + cLENGTH + (1|LEVEL) + (1|FEMALE),
              family=student(),
              data = growth2, 
              warmup = 4000, 
              iter = 10000,
...
summary(model1.tsqrt.student )
pp_check(model1.tsqrt.student , type = 'dens_overlay')

#--- DHARMa checks ---#
growth3 <- growth2 |> 
  drop_na(HATCHING_SUCCESS)
preds <- posterior_predict(model1.tsqrt.student , ndraws=250, summary=FALSE)
model1.tsqrt.student.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = na.omit(sqrt(growth3$cMASS)), 
                              fittedPredictedResponse = apply(preds, 2, median), 
                              integerResponse = 'student')
plot(model1.tsqrt.student.resids) ; testDispersion(model1.tsqrt.student.resids)
``` 
...
## Basic model
```{r fit-model-1, cache=TRUE}
model1.mass <- brm(MASS ~ DEV_TEMP*REGION + 
...
```{r r-squared}
model1.tsqrt.student |> bayes_R2(summary = FALSE) |> median_hdci()
```

## tidyMCMC 

```{r tidy}
tidyMCMC(model1.tsqrt.student, estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
model1.tsqrt.student |> gather_draws(`b_.*|sigma`, regex =TRUE) |> 
  median_hdci()
``` 
...
```{r bayesplot-1}
model1.tsqrt.student |> mcmc_plot(type='intervals')
```

## emmeans - pariwise 

```{r emmeans-pairwise}
model1.tsqrt.student |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="DEV_TEMP") |> summary()
```

## probabilities 
```{r probabilities}
#model1.tsqrt.student |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 31.5, length.out = 10)) |> pairs(type = 'response')
mtsqst <- model1.tsqrt.student |> emmeans(pairwise ~ REGION*DEV_TEMP)
...
var <- get_variables(model1.tsqrt.student)
var1 <- get_variables(model1.tsqrt.student)[c(1:4,8:9)] ; var1 

int_draws_spread <- model1.tsqrt.student |> spread_draws(!!!syms(var1)) #|> 
  #mutate(b_Intercept = b_Intercept^2, 
        # b_DEV_TEMP30 = b_DEV_TEMP30^2, 
        # b_DEV_TEMP31.5 = b_DEV_TEMP31.5^2, 
        # b_REGIONleading = b_REGIONleading^2, 
         #`b_DEV_TEMP30:REGIONleading` = `b_DEV_TEMP30:REGIONleading`^2, 
         #`b_DEV_TEMP31.5:REGIONleading` = `b_DEV_TEMP31.5:REGIONleading`^2)
