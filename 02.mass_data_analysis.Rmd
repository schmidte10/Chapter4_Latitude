...
### Model3.3 (geographical) {.tabset}
#### Model
```{r fit-model3.3, cache=TRUE}
f.model3.3 <- bf(MASS ~ 1 + 
                         DEV_TEMP*REGION + 
                         scale(FEMALE_MASS, center=TRUE, scale=TRUE) + 
                         scale(DENSITY, center=TRUE, scale=TRUE) + 
                         scale(AGE_DAYS, center=TRUE, scale=TRUE) +
...
model3.3 <- brm(f.model3.3,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95)) 
saveRDS(model3.3, file="./02.mass_data_analysis_files/model3.3.RDS")
```

#### DHARMa
```{r model3.3-DHARMa}
#--- distribution check ---#
pp_check(model3.3, type = 'dens_overlay')

#--- DHARMa checks ---#
preds <- posterior_predict(model3.3, ndraws=250, summary=FALSE)
model3.3_resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = growth3$MASS, 
                              fittedPredictedResponse = apply(preds, 2, mean), 
                              integerResponse = 'student')
plot(model3.3_resids) ; testDispersion(model3.3_resids)
```

#### MCMC Diagnostics

```{r model3.3-mcmc-diag, fig.height=9}
stan_trace(model3.3$fit)
stan_ac(model3.3$fit) 
stan_rhat(model3.3$fit) 
stan_ess(model3.3$fit)
stan_dens(model3.3$fit, separate_chains = TRUE)
```
...
LOO(model3.1, model3.2, model3.3) 
```
...
model3.3_gamma1.priors <- prior(normal(1.13, 0.20), class="Intercept") +  
  prior(normal(0, 0.25), class="b")
```

## Model3.3_gamma1 {.tabset}
### Model
```{r model3.3_gamma1, cache=TRUE}
f.model3.3_gamma1 <- bf(MASS ~ 1 + 
                         DEV_TEMP*REGION + 
                         scale(FEMALE_MASS, center=TRUE, scale=TRUE) + 
                         scale(DENSITY, center=TRUE, scale=TRUE) + 
                         scale(AGE_DAYS, center=TRUE, scale=TRUE) +
...
model3.3_gamma1 <- brm(f.model3.3_gamma1,
              data = growth3, 
              prior = model3.3_gamma1.priors,
              warmup = 500, 
              iter = 5000,
              seed=123, 
              cores=2, 
              ...
              thin = 5, 
              control = list(adapt_delta=0.95))  
saveRDS(model3.3_gamma1, file="./02.mass_data_analysis_files/model3.3_gamma1.RDS")
```

### DHARMa
```{r model3.1_gamma1-DHARMa}
#--- distribution check ---#
pp_check(model3.3_gamma1, type = 'dens_overlay')

#--- DHARMa checks ---#
preds <- posterior_predict(model3.3_gamma1, ndraws=250, summary=FALSE)
model3.3_gamma1_resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = growth3$MASS, 
                              fittedPredictedResponse = apply(preds, 2, mean), 
                              integerResponse = 'student')
plot(model3.3_gamma1_resids) ; testDispersion(model3.3_gamma1_resids); testDispersion(model3.1_resids, plot = F, alternative = "greater")
```

### MCMC Diagnostics

```{r model3.3_gamma1-mcmc-diag, fig.height=9}
stan_trace(model3.3_gamma1$fit)
stan_ac(model3.3_gamma1$fit) 
stan_rhat(model3.3_gamma1$fit) 
stan_ess(model3.3_gamma1$fit)
stan_dens(model3.3_gamma1$fit, separate_chains = TRUE)
```
...
```{r r-squared}
model3.3_gamma1 |> bayes_R2(summary = FALSE) |> median_hdci()
```

## tidyMCMC 

```{r tidy}
tidyMCMC(model3.3_gamma1, estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
model3.3_gamma1 |> gather_draws(`b_.*|sigma`, regex =TRUE) |> 
  median_hdci()
``` 
...
```{r bayesplot-1}
model3.3_gamma1 |> mcmc_plot(type='intervals')
```

## emmeans - pariwise 

```{r emmeans-pairwise}
model3.3_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="DEV_TEMP") |> summary(infer=TRUE)

model3.3_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="REGION") |> confint()

model3.3_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() 
...
mtsqst <- model3.3_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP)
...
var <- get_variables(model3.3_gamma1)
var1 <- get_variables(model3.3_gamma1)[c(1:4,10,11)] 

int_draws_spread <- model3.3_gamma1 |> spread_draws(!!!syms(var1)) 
