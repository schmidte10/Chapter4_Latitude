...
model3.1_gamma1 |> emmeans(pairwise ~ DEV_TEMP, type="response")  
```
...
mass.plot <- int_draws_plotting |>
  ...
  scale_y_continuous(limits=c(0.75,1.25), breaks = seq(0.75,1.25, 0.25))+
...
```{r}
model3.1.nolength <- readRDS("02.mass_data_analysis_files/model3.1_growth_nolength.RDS")
```
...
```{r emmeans-pairwise}
out <- model3.1.nolength  |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response")  
out$emmeans |>
  as.data.frame() 

model3.1.nolength  |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid()|> pairs(by="REGION") |> summary(infer=TRUE)

model3.1.nolength |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="DEV_TEMP") |> summary(infer=TRUE)

model3.1.nolength  |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() |> pairs() |> summary(infer=TRUE) 

model3.1.nolength  |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() 

```
## probabilities 
```{r probabilities}
#model1.geo.gamma |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 31.5, length.out = 10)) |> pairs(type = 'response')
mtsqst <- model3.1.nolength |> emmeans(pairwise ~ REGION*DEV_TEMP)
mtsqrt2 <- mtsqst$contrasts |> gather_emmeans_draws()
mtsqrt2 %>% group_by(contrast) %>% dplyr::summarise(Prob = sum(.value<0)/n())
```
...
mass.plot <- int_draws_plotting |> 
  ...
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              ...
                              "High latitude_DEV_TEMP_30" = paste0("High latitude 30","\u00B0","C"),
                              "High latitude_DEV_TEMP_31.5" = paste0("High latitude 31.5","\u00B0","C"))) +
  annotate("text", x=0.6,y=1.25, label = paste0(round(median(int_draws_plotting$MASS), 2)," (g)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
...
mass.plot <- mass_int_draws_plotting |> 
  ggplot(aes(x=EXP_GROUP, y=exp(MASS))) + 
  stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), 
               point_interval = median_hdci, 
               .width = c(.66, .89, .95, .99)) + 
  scale_fill_ramp_discrete(na.translate=FALSE, 
                           ...
                           name = "Credible interval") +
  scale_fill_manual(values = c("lightskyblue" ,"dodgerblue2", "dodgerblue4","coral", "red2","firebrick4"))+ 
  scale_y_continuous(limits=c(0.5,2), breaks = seq(0.5,2, .5))+
  ylab("MASS (g)") + xlab("EXPERIMENTAL GROUP") +
  geom_hline(yintercept = median(exp(mass_int_draws_plotting$MASS)), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) +
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              "Low latitude_DEV_TEMP_30" = paste0("Low latitude 30","\u00B0","C"),
                              "Low latitude_DEV_TEMP_31.5" = paste0("Low latitude 31.5","\u00B0","C"), 
                              "High latitude_DEV_TEMP_28.5" = paste0("High latitude 28.5","\u00B0","C"),
...
condition.plot <- int_draws_plotting |> 
  ...
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              ...
                              "High latitude_DEV_TEMP_30" = paste0("High latitude 30","\u00B0","C"),
                              "High latitude_DEV_TEMP_31.5" = paste0("High latitude 31.5","\u00B0","C"))) +
  annotate("text", x=0.7,y=1.09, label = paste0(round(median(int_draws_plotting$MASS), 2)," (g)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
  theme(legend.position = "none"); condition.plot 
        #axis.title.y = element_text(margin = margin(r =0.3, unit = "in"), size = 12), 
        #axis.title.x = element_text(margin = margin(t = 0.3, unit="in"), size =12), 
        #legend.key = element_rect(color="black", size=1.25), 
...
ggsave(file="./figures/figure2_v2.jpeg",  plot=figure2 , width=6, height=12, units = "in", dpi=300) 
ggsave(file="./figures/figure2_v2.pdf",  plot=figure2 , width=6, height=12, units = "in", dpi=1200)
```
