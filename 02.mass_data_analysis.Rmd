...
### Priors 
```{r priors-re0.2}
model.re0.2.formula <- bf(MASS ~ 1 + (1|TANK) + (1|FEMALE))
get_prior(model.re0.2.formula, data=growth3)
...
saveRDS(model.re0.2, file="./02.mass_data_analysis_files/model.randomfactors0.2.RDS")
```
...
### Priors 
```{r priors-re0}
model.re0.formula <- bf(MASS ~ 1 + (1|LEVEL/TANK) + (1|FEMALE))
get_prior(model.re0.formula, data=growth3)
mass.priors.re <- prior(normal(1.13, 0.20), class="Intercept") +  
  prior(student_t(3, 0, 0.53), class = "sigma")
```
...
saveRDS(model.re0, file="./02.mass_data_analysis_files/model.randomfactors0.RDS")
```
...
saveRDS(model.re2, file="./02.mass_data_analysis_files/model.randomfactorsre2.RDS")
```
...
### Collinearity 
```{r }
model1.lm <- lm(MASS ~ 1 + DEV_TEMP+REGION + 
...
saveRDS(model.mat, file="./02.mass_data_analysis_files/model1.maternal.RDS")
```
...
model1.geo.formula <- bf(MASS ~ 1 + 
                         ...
                         (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE), 
                         family=gaussian())
get_prior(model1.geo.formula, data=growth3)
mass.priors <- prior(normal(1.13, 0.20), class="Intercept") +  
  prior(normal(0, 0.25), class="b") +
  prior(student_t(3, 0, 0.53), class = "sigma")
```
...
model1.geo <- brm(model1.geo.formula,
              data = growth3, 
              prior = mass.priors,
              warmup = 500, 
              iter = 5000,
              ...
              thin = 5, 
              control = list(adapt_delta=0.95)) 
prior_summary(model1.geo) 
saveRDS(model.mat, file="./02.mass_data_analysis_files/model1.geographic.RDS")
```

### stan plots
```{r mcmc-diag-model3-2, fig.height=9}
stan_trace(model1.geo$fit)
stan_ac(model1.geo$fit) 
stan_rhat(model1.geo$fit) 
stan_ess(model1.geo$fit)
stan_dens(model1.geo$fit, separate_chains = TRUE)

hypothesis(model1.geo,hypothesis ="DEV_TEMP31.5 < DEV_TEMP30")
plot(hypothesis(model1.geo, "DEV_TEMP30:REGIONleading<REGIONleading"))
```
...
```{r model1.mass}
#--- distribution check ---#
pp_check(model1.mat, type = 'dens_overlay')

#--- DHARMa checks ---#
preds <- posterior_predict(model1.geo, ndraws=250, summary=FALSE)
model1.geo.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = growth3$MASS, 
                              fittedPredictedResponse = apply(preds, 2, mean), 
                              integerResponse = 'student')
plot(model1.geo.resids) ; testDispersion(model1.geo.resids)
```


There are issues with this model lets fit it using a Gamma distribution. 

**Note:** The Gamma distribution will have a default log link function. 
# Model fit

## Priors and formula
```{r priors-geo.gamma}
model1.geo.gamma.formula <- bf(MASS ~ 1 + 
                         DEV_TEMP*REGION + 
                         scale(MALE_MASS, center=TRUE, scale=TRUE) + 
                         scale(DENSITY, center=TRUE, scale=TRUE) + 
                         scale(AGE_DAYS, center=TRUE, scale=TRUE) +
                         ...
                         (1|LEVEL/TANK) + (1|POPULATION) + (1|FEMALE), 
                         family=Gamma())
get_prior(model1.geo.gamma.formula, data=growth3)
geo.gamma.priors <- prior(normal(1.13, 0.20), class="Intercept") +  
  prior(normal(0, 0.25), class="b")
```

## Model1.geo.gamma

```{r model1.geo.gamma, cache=TRUE}
model1.geo.gamma <- brm(model1.geo.gamma.formula,
              data = growth3, 
              prior = geo.gamma.priors,
              warmup = 500, 
              iter = 5000,
              seed=123, 
              cores=2, 
...
summary(model1.geo.gamma)
saveRDS(model1.geo.gamma, file="./02.mass_data_analysis_files/model1.geo.gamma.RDS")
```

## stan plots
```{r mcmc-diag-geo-gamma, fig.height=9}
stan_trace(model1.geo.gamma$fit)
stan_ac(model1.geo.gamma$fit) 
stan_rhat(model1.geo.gamma$fit) 
stan_ess(model1.geo.gamma$fit)
stan_dens(model1.geo.gamma$fit, separate_chains = TRUE)
```

# Model validation

```{r model-diag-model1.geo.gamma}
#--- distribution check ---#
pp_check(model1.geo.gamma, type = 'dens_overlay')

#--- DHARMa checks ---#
preds <- posterior_predict(model1.geo.gamma, ndraws=250, summary=FALSE)
model1.geo.resids <- createDHARMa(simulatedResponse = t(preds), 
                              observedResponse = growth3$MASS, 
                              fittedPredictedResponse = apply(preds, 2, mean), 
                              integerResponse = 'gamma')
plot(model1.geo.resids) ; testDispersion(model1.geo.resids)
...
```{r r-squared}
model1.geo.gamma |> bayes_R2(summary = FALSE) |> median_hdci()
```

## tidyMCMC 

```{r tidy}
tidyMCMC(model1.geo.gamma, estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
model1.geo.gamma |> gather_draws(`b_.*|sigma`, regex =TRUE) |> 
  median_hdci()
``` 
...
```{r bayesplot-1}
model1.geo.gamma |> mcmc_plot(type='intervals')
```

## emmeans - pariwise 

```{r emmeans-pairwise}
model1.geo.gamma |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="DEV_TEMP") |> summary(infer=TRUE)

model1.geo.gamma |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> pairs(by="REGION") |> confint()

model1.geo.gamma |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() 
...
mtsqst <- model1.geo.gamma |> emmeans(pairwise ~ REGION*DEV_TEMP)
...
var <- get_variables(model1.geo.gamma); var
var1 <- get_variables(model1.geo.gamma)[c(1:4,10,11)] ; var1 

int_draws_spread <- model1.geo.gamma |> spread_draws(!!!syms(var1)) #|> 
