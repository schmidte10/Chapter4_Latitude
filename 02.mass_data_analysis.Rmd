...
```{r model3.1, cache=TRUE, eval=FALSE, echo=FALSE}
...
model3.1 <- readRDS("02.mass_data_analysis_files/model3.1.RDS")
```
...
#### Model
```{r model3.2, cache=TRUE, eval=FALSE}
...
```{r eval=TRUE, echo=FALSE}
model3.2 <- readRDS("02.mass_data_analysis_files/model3.2.RDS")
```
...
### Model
```{r model3.3_gamma1, cache=TRUE, eval=FALSE, echo=FALSE}
...
```{r echo=FALSE}
model3.1_gamma1 <- readRDS("02.mass_data_analysis_files/model3.1_gamma1.RDS")
```
...
```{r emmeans-pairwise2}
out <- model3.1_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") 
out$emmeans |>
  as.data.frame()

model3.1_gamma1 |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() |> pairs() |> summary(infer=TRUE)

model3.1_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="DEV_TEMP") |> summary(infer=TRUE)

model3.1_gamma1 |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid()|> pairs(by="REGION") |> summary(infer=TRUE)
...
```{r summary-figure-1, warning=FALSE, message=FALSE, echo =TRUE, eval =FALSE}
...
## Model3.1_gamma1 {.tabset}
### Model
```{r model3.3_growth-sio, cache=TRUE, eval=FALSE}
...
```{r}
model3.1_growth_nolength <- readRDS("C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/Chapter4_Latitude/02.mass_data_analysis_files/model3.1_growth_nolength.RDS")
```
...
```{r emmeans-pairwise}
out <- model3.1_growth_nolength  |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response")  
out$emmeans |>
  as.data.frame() 

model3.1_growth_nolength  |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid()|> pairs(by="REGION") |> summary(infer=TRUE)

model3.1_growth_nolength |> emmeans(pairwise ~ REGION*DEV_TEMP, type="response") |> regrid() |> pairs(by="DEV_TEMP") |> summary(infer=TRUE)

model3.1_growth_nolength  |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() |> pairs() |> summary(infer=TRUE) 

model3.1_growth_nolength  |> emmeans(pairwise ~ DEV_TEMP, type="response") |> regrid() 

```
## probabilities 
```{r probabilities}
#model1.geo.gamma |> emmeans(~ REGION | DEV_TEMP, at = list(DEV_TEMP = 31.5, length.out = 10)) |> pairs(type = 'response')
mtsqst <- model3.1_growth_nolength |> emmeans(pairwise ~ REGION*DEV_TEMP)
mtsqrt2 <- mtsqst$contrasts |> gather_emmeans_draws()
mtsqrt2 %>% group_by(contrast) %>% dplyr::summarise(Prob = sum(.value<0)/n())
```
...
```{r summary-figure-2, warning=FALSE, message=FALSE, eval=FALSE}
...
mass.plot <- int_draws_plotting |> 
  ...
  ylab("MASS (g)") + xlab("EXPERIMENTAL GROUP") +
  geom_hline(yintercept = median(mass_int_draws_plotting$MASS), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) +
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              "Low latitude_DEV_TEMP_30" = paste0("Low latitude 30","\u00B0","C"),
                              "Low latitude_DEV_TEMP_31.5" = paste0("Low latitude 31.5","\u00B0","C"), 
                              "High latitude_DEV_TEMP_28.5" = paste0("High latitude 28.5","\u00B0","C"),
                              "High latitude_DEV_TEMP_30" = paste0("High latitude 30","\u00B0","C"),
                              "High latitude_DEV_TEMP_31.5" = paste0("High latitude 31.5","\u00B0","C"))) +
  annotate("text", x=0.6,y=1.25, label = paste0(round(median(mass_int_draws_plotting$MASS), 2)," (g)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
  theme(legend.position = "none"); mass.plot
        #axis.title.y = element_text(margin = margin(r =0.3, unit = "in"), size = 12), 
        #axis.title.x = element_text(margin = margin(t = 0.3, unit="in"), size =12), 
        #legend.key = element_rect(color="black", size=1.25), 
...
# Figure 2 
model1 <- readRDS("C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/Chapter4_Latitude/01.growth_data_analysis_files/model1.length.RDS") 
...
growth3 <- growth2 |> 
  filter(MASS < 4, na.rm=TRUE, 
         ...
         FULTONK > 0.01, 
         DENSITY > 3)
model3.1_gamma1 <- readRDS("C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/Chapter4_Latitude/02.mass_data_analysis_files/model3.1_gamma1.RDS") 
model3.1.nolength <- readRDS("C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/Chapter4_Latitude/02.mass_data_analysis_files/model3.1_growth_nolength.RDS")
```
...
length_int_draws_plotting <- int_draws |> 
  pivot_longer(cols = starts_with("x"), 
               names_to = "DEV_TEMP", 
               values_to = "LENGTH") |> 
  transmute(LATITUDE = case_when(`.variable` == "b_Intercept" ~ "Low latitude", 
...
length.plot <- length_int_draws_plotting |> 
  ggplot(aes(x=EXP_GROUP, y=LENGTH)) + 
  stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), 
               point_interval = median_hdci, 
               .width = c(.66, .90, .95, .99)) + 
  scale_fill_ramp_discrete(na.translate=FALSE, 
                           labels =c("0.99","0.95","0.90","0.66"), 
                           name = "Credible interval") +
  scale_fill_manual(values = c("lightskyblue" ,"dodgerblue2", "dodgerblue4","coral", "red2","firebrick4"))+ 
  scale_y_continuous(limits=c(25,35), breaks = seq(25,35, 2))+
  ylab("STANDARD LENGTH (mm)") + xlab("EXPERIMENTAL GROUP") +
  geom_hline(yintercept = median(length_int_draws_plotting$LENGTH), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) +
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              "Low latitude_DEV_TEMP_30" = paste0("Low latitude 30","\u00B0","C"),
                              "Low latitude_DEV_TEMP_31.5" = paste0("Low latitude 31.5","\u00B0","C"), 
                              "High latitude_DEV_TEMP_28.5" = paste0("High latitude 28.5","\u00B0","C"),
                              "High latitude_DEV_TEMP_30" = paste0("High latitude 30","\u00B0","C"),
                              "High latitude_DEV_TEMP_31.5" = paste0("High latitude 31.5","\u00B0","C"))) +
  annotate("text", x=0.6,y=33, label = paste0(round(median(length_int_draws_plotting$LENGTH), 2)," (mm)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
  theme(legend.position = c(.18,.86), 
        axis.title.y = element_text(margin = margin(r =0.3, unit = "in"), size = 12), 
        axis.title.x = element_text(margin = margin(t = 0.0, unit="in"), size =12), 
...
mass_int_draws_plotting <- int_draws |> 
  pivot_longer(cols = starts_with("x"), 
               names_to = "DEV_TEMP", 
               values_to = "MASS") |> 
  transmute(LATITUDE = case_when(`.variable` == "b_Intercept" ~ "Low latitude", 
  ...
  mutate(EXP_GROUP = factor(EXP_GROUP, levels=c("High latitude_DEV_TEMP_28.5","High latitude_DEV_TEMP_30", "High latitude_DEV_TEMP_31.5",
                                                "Low latitude_DEV_TEMP_28.5","Low latitude_DEV_TEMP_30", "Low latitude_DEV_TEMP_31.5")))
median(mass_int_draws_plotting$MASS)

mass.plot <- mass_int_draws_plotting |> 
  ggplot(aes(x=EXP_GROUP, y=MASS)) + 
  stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), 
               point_interval = median_hdci, 
               .width = c(.66, .89, .95, .99)) + 
...
condition.plot <- int_draws_plotting |> 
  ggplot(aes(x=EXP_GROUP, y=MASS)) + 
  stat_halfeye(aes(fill = EXP_GROUP, fill_ramp = after_stat(level)), 
               point_interval = median_hdci, 
               .width = c(.66, .89, .95, .99)) + 
  ...
  scale_fill_manual(values = c("lightskyblue" ,"dodgerblue2", "dodgerblue4","coral", "red2","firebrick4"))+ 
  scale_y_continuous(limits=c(1,5), breaks = seq(1,5, 1))+
  ylab("BODY CONDITION (g)") + xlab("EXPERIMENTAL GROUP") +
  geom_hline(yintercept = median(int_draws_plotting$MASS), linetype="dashed", linewidth=1, color="grey58", alpha=0.8) +
  scale_x_discrete(labels = c("Low latitude_DEV_TEMP_28.5" = paste0("Low latitude 28.5","\u00B0","C"), 
                              "Low latitude_DEV_TEMP_30" = paste0("Low latitude 30","\u00B0","C"),
                              "Low latitude_DEV_TEMP_31.5" = paste0("Low latitude 31.5","\u00B0","C"), 
                              "High latitude_DEV_TEMP_28.5" = paste0("High latitude 28.5","\u00B0","C"),
                              "High latitude_DEV_TEMP_30" = paste0("High latitude 30","\u00B0","C"),
                              "High latitude_DEV_TEMP_31.5" = paste0("High latitude 31.5","\u00B0","C"))) +
  annotate("text", x=0.6,y=3.3, label = paste0(round(median(int_draws_plotting$MASS), 2)," (g)"), color="grey58") +
  coord_flip() + 
  theme_classic() + 
  guides(fill = "none") + 
  theme(legend.position = "none") 
        ...
        #axis.title.x = element_text(margin = margin(t = 0.3, unit="in"), size =12), 
        #legend.key = element_rect(color="black", size=1.25), 
        #legend.background = element_rect(fill = alpha("blue", 0))); condition.plot
```



```{r fig.width=7.5, fig.height=14, eval=FALSE, echo =FALSE}
## plot final figure 
figure2 <- length.plot / mass.plot / condition.plot + 
  plot_annotation(tag_levels ="A") 

#ggsave(file="./figures/figure2.jpeg",  plot=figure2 , width=6, height=12, units = "in", dpi=300) 
#ggsave(file="./figures/figure2.pdf",  plot=figure2 , width=6, height=12, units = "in", dpi=1200)
```
