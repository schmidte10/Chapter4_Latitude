...
```{r eda}
ggplot(growth3, aes(x=DEV_TEMP, y=MASS, color=FEMALE, fill=FEMALE)) + 
...
# Model1.1
```{r model1.1}
model1.1 <- lmer(MASS ~ as.numeric(DEV_TEMP) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION), 
                 REML=FALSE,
                 data=growth3)   
...
#Model1.2
```{r model1.2}
model1.2 <- lmer(MASS ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE)+ scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION), 
                 REML=FALSE,
                 data=growth3)  
...
# Model1.3
```{r model1.3}
model1.3 <- lmer(MASS ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1 |FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
ggplot(dev_temp_pred, aes(x = DEV_TEMP, y = fit1.3)) +
  geom_jitter(data = growth3, aes(y = pred_geno1.3, colour = REGION), width=0.2) +
  geom_smooth(data = growth3, 
            aes(y=MASS, group=FEMALE, color=FEMALE), 
            method = "lm")  +
  geom_line(size = 2)+ 
  theme_classic()
```
...
# Model1.4 
```{r}
model1.4 <- lmer(MASS ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE)+ scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1+DEV_TEMP|FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
ggplot(dev_temp_pred, aes(x = DEV_TEMP, y = fit1.4)) +
  geom_jitter(data = growth3, aes(y = pred_geno1.4, colour = REGION), width=0.2) +
...
# Model1.5 
```{r warning=FALSE}
model1.5 <- lmer(MASS ~ poly(as.numeric(DEV_TEMP), 2, raw = TRUE) + scale(DENSITY, scale=TRUE, center=TRUE)  + (1 |REGION) + (1+DEV_TEMP + I(DEV_TEMP^2)|FEMALE), 
                 REML=FALSE,
                 data=growth3) 
...
ggplot(dev_temp_pred, aes(x = DEV_TEMP, y = fit1.5)) +
  geom_jitter(data = growth3, aes(y = pred_geno1.5, colour = REGION), width=0.2) +
  stat_smooth(data = growth3, 
            aes(y=MASS, group=FEMALE, color=FEMALE), 
            method="lm", 
            formula=y~poly(x, 3, raw=TRUE), 
...
# AIC comparison
AIC(model1.1, model1.2, model1.3, model1.4, model1.5, k=2)
```
...
ggplot(genotype_data, aes(genotype_ordered, BLUP_int)) +
  ...
  ylab("Plasticity (BLUP intercept estimate)") +
...
```{r}
pdf("./figures/S_BLUPSoutput_mass.pdf", width = 8, height =5)
ggplot(genotype_data, aes(genotype_ordered, BLUP_int)) +
  geom_bar(stat = "identity", aes(group = genotype, fill = pop)) +
  xlab("Genotype (in ranked order of plasticity)") +
  ylab("Plasticity (BLUP intercept estimate)") +
