...
FISH_ID = "96.CARL.233.30.2"
mass = 0.001570 #in kilograms 
chamber = "ch4" 
salinity = 37 
chamber_vol = 0.07160
temperature = 30
...
firesting <- read_delim("C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/2023/2023_CTmaxResp/Dell/Experiment_ 14 July 2023 10 01AM/Oxygen data raw/firesting.txt", 
                             delim = "\t", escape_double = FALSE, 
                             col_types = cols(`...2` = col_time(format = "%H:%M:%S")), 
                             trim_ws = TRUE, skip = 18) 
firesting <- firesting[-1,]

pre_bg_dir = "C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/2023/2023_CTmaxResp/Dell/Experiment_ 14 July 2023 09 20AM/All slopes" 
post_bg_dir = "C:/Users/jc527762/OneDrive - James Cook University/PhD dissertation/Data/2023/2023_CTmaxResp/Dell/Experiment_ 14 July 2023 05 52PM/All slopes"
```
...
```{r data_manipulation-multi-inspect, hide=TRUE, warning=FALSE}
...
inspect(firesting3, time = 1, oxygen = 4:5, add.data = "Temperature" )
```
...
exp_start <- which(firesting2$TIME == Tstart)
exp_end <- which(firesting2$TIME == Tend)


apoly_insp <- firesting2 |> 
  subset_data(from = exp_start, 
              to = exp_end, 
              by="row") 
```
...
ggplot(RMR.raw, aes(x=time, y=rate.output*-1)) + 
  geom_point(color = "red", size = 2) + 
  geom_point(data = RMR.filtered, aes(x=time, y=rate.output*-1), 
...
ctmax.resp <- RMR.filtered |> 
  ...
  mutate(time_lag_sec = time - time[1], 
         ...
         sampleID = FISH_ID, 
         exp_date = Date1)%>% 
  select(32,31,1:7,29,30,8:26,28,27) |> 
